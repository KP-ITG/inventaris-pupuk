                ...
                <div class="relative bg-white rounded-lg shadow-xl max-w-6xl w-full max-h-[90vh] overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-gray-50">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 mt-1">Total: {{ (stocks.data || stocks).length }} data akan di-export</p>
                        </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200 border">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Lokasi Gudang</th>
                                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Status</th>
                                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Update Terakhir</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="(item, index) in (stocks.data || stocks)" :key="item.id" class="hover:bg-gray-50">
                                        <td class="px-4 py-2 text-sm text-gray-900">{{ index + 1 }}</td>
                                        <td class="px-4 py-2 text-sm text-gray-900">{{ item.pupuk?.nama_pupuk }}</td>
                                        <td class="px-4 py-2 text-sm text-gray-900">{{ item.pupuk?.kode_pupuk || '-' }}</td>
                                        <td class="px-4 py-2 text-sm text-gray-900">{{ item.jumlah_stok }} kg</td>
                                        <td class="px-4 py-2 text-sm text-gray-900">{{ item.stok_minimum || 0 }} / {{ item.stok_maksimum || 0 }} kg</td>
                                        <td class="px-4 py-2 text-sm text-gray-900">{{ item.lokasi_gudang || '-' }}</td>
                                        <td class="px-4 py-2 text-sm">
                                            <span :class="['px-2 inline-flex text-xs leading-5 font-semibold rounded-full', getStatusClass(item.status_stok)]">
                                                {{ getStatusLabel(item.status_stok) }}
                                            </span>
                                        </td>
                                        <td class="px-4 py-2 text-sm text-gray-900">{{ formatDate(item.updated_at) }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
